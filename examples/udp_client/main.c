...
int main(int argc, char * argv[]) {
    ...
    int iRet = sendto(fd, szBuffer, length, 0,
    ...
    iRet = recvfrom(fd, szBuffer, MAX_BUF_SIZE, 0,
    ...
    if (response.reCode != 0) {
        ...
        unsigned int uiReservedLen =
            *(unsigned int *) (response.reservedBuffer);

        printf("uiSeq:%u\tuiUrlType:%u\tuiEvilType:%u\tuiLevel:%u\t"
               "parameter:%s\tuiReservedLen:%u\treservedBuffer:%s\n",
               response.uiSeq, response.uiUrlType, response.uiEvilType,
               response.uiLevel, response.parameter, uiReservedLen,
               response.reservedBuffer +4);
    }
    return 0;
}
