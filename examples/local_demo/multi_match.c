...
int main(int argc, char * argv[]) {
    ...
    while (!feof(fpr)) {
        ...
        if (fgets(buf, sizeof (buf), fpr) != NULL) {
            ...
            int iRet = UrlLibDetect(ptrUrl, &uiUrlType, &uiEvilType, &uiLevel,
                                    szParameter);
            if (iRet != 0x00 && iRet != 0x52) {
                snprintf(resultData, sizeof(resultData),
                         "%s\tUrlLibDetect url failed! iRet:%#x\n",
                         ptrUrl, iRet);
                fputs(resultData, fpw);
            } else {
                snprintf(resultData, sizeof(resultData),
                         "%s\tUrlType: %d\tEvilType: %d\tiLevel: %d\n",
