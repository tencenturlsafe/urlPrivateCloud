...
static void afterMatchHandle(uv_work_t * req, int status) {
    ...
    uv_udp_send(&(pContext->send_req), pContext->handle, &buf, 1,
                (const struct sockaddr *)&(pContext->client_addr), after_send);
}
...
static void matchHandle(uv_work_t * req) {
    struct context * pContext = (struct context *)req->data;
    stRequest request;
    register int iRet = decodeReq((const unsigned char *)pContext->szBuf,
                                  pContext->bufLen, &request);
    if (0 != iRet) {
        ...
        pContext->bufLen = encodeResp(&responseDefault,
                                      (unsigned char *)pContext->szBuf,
                                      MAX_BUF_LEN);
    } else {
        stResponse response;
        memset(&response, 0, sizeof (stResponse));
        response.uiSeq = request.uiSeq;
        memcpy(response.reservedBuffer, request.reservedBuffer,
               MAX_RESERVED_LEN);

        register int iRet = UrlLibDetect(request.sourceUrl,
                                         &(response.uiUrlType),
                                         &(response.uiEvilType),
                                         &(response.uiLevel),
                                         response.parameter);
        ...
        pContext->bufLen = encodeResp(&response,
                                      (unsigned char *)pContext->szBuf,
                                      MAX_BUF_LEN);
    }
}
...
static void recv_cb(uv_udp_t * handle, ssize_t nread, const uv_buf_t * buf,
    ...
    if (nread <= 0) {
        ...
        memcpy((void *)&pContext->client_addr, (const void *)addr,
               sizeof(struct sockaddr));

        free(buf->base);

        (pContext->work_req).data = (void *)pContext;
        uv_queue_work(handle->loop, &(pContext->work_req),
                      matchHandle, afterMatchHandle);
    }
}
...
static void threadHandle(void * arg) {
    ...
    iRet = uv_udp_bind(&udp_sv, (const struct sockaddr *)&addr,
                       UV_UDP_REUSEADDR);
    if (iRet) {
...
bool startThread(const char * szConfFile) {
    ...
    if (1 == confInfo.uiType) {
        int iErrCode = 0;
        if (!UrlLibInit(confInfo.uiShmKey, confInfo.uiSize,
                        confInfo.strLicence, &iErrCode)) {
            LOG("UrlLibInit Failed! iErrCode:%#x", iErrCode);
            return false;
        }
    } else if (2 == confInfo.uiType) {
        int iErrCode = 0;
        if (!UrlLibInitByMb(confInfo.uiShmKey, confInfo.uiSize,
                            confInfo.strLicence, &iErrCode)) {
            LOG("UrlLibInitByMb Failed! iErrCode:%#x", iErrCode);
            return false;
        }
    } else {
        LOG("Error initType! (1 for UrlLibInit and 2 for UrlLibInitByMb)");
        return false;
    }
    ...
    for (; uiThread < confInfo.threadNum && uiThread < MAX_THREAD_NUM;
         ++uiThread) {
        int iRet = uv_thread_create(&thread_id[uiThread], threadHandle,
                                    &confInfo.port[uiThread]);
        if (iRet) {
